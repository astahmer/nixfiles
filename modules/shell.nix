...
in
{
  config.flake.modules.homeManager.shell =
    ...
    in
    {
      ...
      # Prebuilt upstream binaries: nixpkgs' source build has no darwin cache hits
      # and burns ~35m compiling Rust on every bump.
      programs.mise = {
        package = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.mise;
        enable = true;
        # Hooks are generated by shellInteractive so activation does not
        # spawn mise to produce the same script for every new shell.
