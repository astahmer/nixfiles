...
- no-domain-effect-run or no-domain-run-promise: domain code preserves Effect
  programs; run them only at application or adapter boundaries.
...
- no-service-flat-map-facade: avoid static service facades that repeatedly
  flatMap the same service; expose the service operation directly.
