...
_secret() {
  ...
  local -a matches
  if (( CURRENT == 2 )); then
    matches=(status unlock lock list search get set add edit id totp source pull sync pin rotate rm delete remove unset mv init env run print global prune lint doctor recent history st ls g s e d pr pu so)
