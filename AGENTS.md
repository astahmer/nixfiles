...
- `scripts/update-pins.json` is the source of truth for routine package/input updates; run it through `nix run .#update-pins`. Keep coupled lockfile or multi-platform hash updates explicitly manual and document them in `docs/UPDATE_COMMANDS.md`. The `agy` binary is Nix-packaged, but ModLens provider authentication is runtime-only; never add `agy` credentials or provider API keys to Nix assets.
