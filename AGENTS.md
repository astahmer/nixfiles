...
- `assets/.agents/skills/antislop/` and `effect-antislop/` contain the executable Oxlint and ast-grep rule sources, fixtures, and selectable profile docs. `modules/coding.nix` installs the matching `oxlint` and `ast-grep` CLIs globally.
