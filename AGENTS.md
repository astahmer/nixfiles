...
- Custom packages live in `packages/<name>/default.nix`, use `finalAttrs`, and are exposed through `perSystem.packages` with `pkgs.callPackage` so `nix-update` can locate them. Prefer upstream release archives over compiling a Rust/Go workspace when the project publishes binaries. `modlens` and `modsearch` are packaged from pinned npm tarballs — modsearch additionally vendors `assets/modsearch/package-lock.json` and pins its upstream skill commit — and their skills are overlaid into the generated `.agents` tree by `modules/agents.nix`.
- `scripts/update-pins.json` is the source of truth for routine package/input updates; run it through `nix run .#update-pins`. Keep coupled lockfile or multi-platform hash updates explicitly manual and document them in `docs/UPDATE_COMMANDS.md`. The `agy` binary is Nix-packaged, but ModLens/ModSearch provider authentication is runtime-only; never add `agy` credentials or provider API keys to Nix assets.
...
- The pi coding agent is Nix-managed by `modules/pi.nix` with sources under `assets/pi/`: the `pi` binary comes from `inputs.llm-agents`, package extensions (`~/.pi/agent/npm/node_modules`) are built from the vendored `assets/pi/npm/package-lock.json` via `buildNpmPackage` (exact version pins in `piSettings.packages`; bump pin + lockfile + `npmDepsHash` together), local extensions and skills are vendored in `assets/pi/extensions` and `assets/pi/skills`. `settings.json` deploys as a write-if-changed activation script because pi mutates it at runtime. Never `pnpm add -g @earendil-works/pi-coding-agent` or edit `~/.pi/agent` in place — change the nixfiles and run `nixapply`. `qmd` (memory search backend for pi-memory) is still installed out-of-band via bun and not yet packaged.
